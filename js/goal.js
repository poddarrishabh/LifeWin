/* The Major Javascript File Behind The Functionality of the entire App */

//It is set to one so that when user uses log in with facebook we candisplay his image.
//otherwise a generic image is displayed.
...
function sync(){
	...
	//var data = "meta="+meta+"&DailyDetails="+DailyDetails+"&DailyNames="+DailyNames+"&GoalsDetails="+GoalsDetails+"&GoalsNames="+GoalsNames+"&TasksDetails="+TasksDetails+"&TasksNames="+TasksNames+"&scoreboard="+scoreboard;
	var data = "log="+log;
	xmlhttp.send(data);
}
...
window.onload=function refresh(){
	...
	if (localStorage.getItem("meta") === null){
		...
		var meta = '{"date":"'+date+'","Present":0,"Future":0,"Today":0,"Tomorrow":0,"This Week":0,"Later":0,"Habits":0,"Frequent":0,"timer":0,"lastTimer":0,"daily":"00","weekly":"00","initialPos":0,"list":"Tasks","Goals":["Present","Future"],"Tasks":["Today","Tomorrow","This Week","Later"],"Dailies":["Habits","Frequent"]}';
		var goalsDetails = '{}';
		var goalsNames = '[]';
		var tasksDetails = '{}';
		var tasksNames = '[]';
...
function list(listname){
	...
	/*To hide the side pane and show the list in full pane each time a list item on the left is clicked*/
	...
	//No form and tags for scoreboard
...
function scoreBoard(){	
	...
	for(var i=keys.length-1; i>=0; i--){
		...
		container.setAttribute('class','list-group-item scoreboard');
...
function loadData(){
	...
	*/
	...
	for(var pos=0; pos<listNames.length; pos++){
		...
		if(listname == "Goals"){
			label.setAttribute("class","plus-sign");
		}
		...
		//Adding a custom class extender to make it easier to be manipulated using javascript and css
		
		node.setAttribute("class","list list-group-item list-design extender");
		...
		if(listname == "Tasks" || listname == "Dailies"){
			$('.edit-points').show();
			$('.edit-points').html('Set Points for this task');
		} else {
			$('.edit-points').hide();
		}
		if(listname!="Dailies"){
		...
		if(listDetails[listNames[pos]].duedate!='')
...
$(document).ready(function() {
	...
	/*Making the calendar icon clickable */
	$('body').on('click','.cal-due',function(){
		$('.duedatepicker').focus();
	});
	$('body').on('click','.cal-rem',function(){
		$('.reminder').focus();
	});
	
		$('.datepicker.duedatepicker').datepicker({
	...
	function enableFuture(date){
		...
		var pos = ($('.activeext').attr('id')).slice(2);
		console.log(pos);
	}
